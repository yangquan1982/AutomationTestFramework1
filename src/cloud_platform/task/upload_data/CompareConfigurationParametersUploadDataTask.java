...
public class CompareConfigurationParametersUploadDataTask {
...
//		IndexPage.netUpdataMange(driver);//老界面
		IndexPage.netUpdataMange(driver);//新界面
		// 等待上传组件加载
		String WinID = CompareConfigurationParametersUploadDataTask.waitUpDataPage(driver, projectName);
		// 选择制式
		CompareConfigurationParametersUploadDataTask.NetType(driver, NetType);
		// 数据类型
		CompareConfigurationParametersUploadDataTask.DataType(driver, DataType);
		// 选择数据子类型
		if (SubDataType != null) {
			CompareConfigurationParametersUploadDataTask.SubDataType(driver,NetType, DataType, SubDataType);
		}

		// 特性选择
		if (FeatureType != null) {
			CompareConfigurationParametersUploadDataTask.FeatureType(driver, SubDataType, FeatureType);
		}
		...
		if (folderName != null) {
			if("不需文件夹".equalsIgnoreCase(folderName.trim())){
				...
				CompareConfigurationParametersUploadDataTask.UpDir(driver, WinID, folderName);
			}
