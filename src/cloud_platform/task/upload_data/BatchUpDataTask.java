...
//		IndexPage.netUpdataMange(driver);//老界面
		IndexPage.netUpdataMange(driver);//新界面
		// 等待上传组件加载
		String WinID = BatchUpDataTask.waitUpDataPage(driver, projectName);
		// 选择制式
		BatchUpDataTask.NetType(driver, NetType);
		// 数据类型
		BatchUpDataTask.DataType(driver, DataType);
		// 选择数据子类型
		if (SubDataType != null) {
			BatchUpDataTask.SubDataType(driver,NetType, DataType, SubDataType);
		}

		// 特性选择
		if (FeatureType != null) {
			BatchUpDataTask.FeatureType(driver, SubDataType, FeatureType);
		}
		...
		if (folderName != null) {
			if("不需文件夹".equalsIgnoreCase(folderName.trim())){
				...
				BatchUpDataTask.UpDir(driver, WinID, folderName);
			}
	...
	public static void DataOtherSubType(WebDriver driver,String NetType, String SubDataType) {
		...
		if ("勘测图片".equalsIgnoreCase(SubDataType)) {
			...
			CommonJQ.click(driver, UploadDataPage.ListSubDataType, "#umtstdd",
					false, "UMTS特征库");
		} else if ("特征库".equalsIgnoreCase(SubDataType)) {
			CommonJQ.click(driver, UploadDataPage.ListSubDataType, "#umtstdd",
					false, "特征库");
	...
	public static void DataSigFeatureType(WebDriver driver, String FeatureType) {
		...
		if ("TopN小区处理,网络评估".equalsIgnoreCase(FeatureType)) {
			...
			CommonJQ.click(driver, UploadDataPage.ListFeatureType,
					"li[id=\"54\"]", true);
		}  else if ("精品覆盖,3D网络评估".equalsIgnoreCase(FeatureType)) {
			CommonJQ.click(driver, UploadDataPage.ListFeatureType,
					"li[id=\"54,114\"]", true);
	...
	public static void DataEnParaFeatureType(WebDriver driver,
		...
		for (int i = 0; i < Type.length; i++) {
			...
			} else if ("基础评估".equalsIgnoreCase(Type[i])) {
				FeatureTypeEN = "Basic Audit";
			...
			} else if ("3D网络评估".equalsIgnoreCase(Type[i])) {
				FeatureTypeEN = "Three Dimensional Network Evaluation";
			} else {
