...
public class T002_WorkSpace {
	...
	public void T02_workspace() {	
		...
		HomeTask.JumpToPersonalWorkspace(driver);
		//审批通过
		WorkSpaceTask.approveApply(driver, WorkSpaceConst.taskdescription03, account1.getAccount(), WorkSpaceConst.ApproveResult1, content);
		HomeTask.logOut(driver);
		LoginTask.login_account_pwd(driver, account1.getAccount(), account1.getPwd());
		HomeTask.JumpToPersonalWorkspace(driver);
		WorkSpaceTask.checkApplyInfor(driver, WorkSpaceConst.taskdescription01, applyreason, ProjectName, WorkSpaceConst.ApplyState3);
