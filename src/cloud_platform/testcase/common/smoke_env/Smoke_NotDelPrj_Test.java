package cloud_platform.testcase.common.smoke_env;
...
public class Smoke_NotDelPrj_Test {
	...
	// Data path
	private static String FilePath = ConstUrl.getProjectHomePath() + "\\Data\\冒烟环境";
	private static String ParaFile = FilePath + "\\工程创建参数化表.xlsx";
	private static String proofile = FilePath + "\\Chrysanthemum.jpg";

	public Smoke_NotDelPrj_Test(String secns, String region, String agent, String operator, String rat,
			String projectNameLabel, String userfile) {
		this.secns = secns;
		this.region = region;
		this.agent = agent;
	...
	public void CreatPrj_NotDel() {
		...
		if (flag) {
			...
			// 创建RF项目
			AwProjectManager.creatProject(driver, projectNameLabel, region, agent, operator, rat, proofile, true);
			// 验证添加成功
