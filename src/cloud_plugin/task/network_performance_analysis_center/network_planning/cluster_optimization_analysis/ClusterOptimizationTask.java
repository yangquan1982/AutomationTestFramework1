package cloud_plugin.task.network_performance_analysis_center.network_planning.cluster_optimization_analysis;
...
public class ClusterOptimizationTask {
	...
	public static String creatDTOptimizTask(WebDriver driver,
		...
		// 打开 DT数据自动分析任务
		NetworkAnalysisCenterTask.openClusterOptimiz(driver);
		...
		ClusterOptimizationTask.setPara(driver, TextRSRP, TextSINR, TextPDCPUL,
				TextPDCPDL);
		ClusterOptimizationTask.chooseFile(driver, defaultWindowID, cfgfile, EParafile,
				DTfile);
	...
	public static String creatThemeOptimizTask(WebDriver driver,
		...
		ClusterOptimizationTask.setThemePara(driver, Mod3Para, OverCoverPara,
	...
	public static String creatCompareReportTask(WebDriver driver,
		...
		LoadingPage.Loading(driver);
		SwitchDriver.switchDriverToFrame(driver, "//iframe");
		CommonJQ.click(driver, AutoDTDataPage.CompareReport_ID, true,"对比报告");
		if (WhetherGrids) {
			CommonJQ.click(driver, AutoDTDataPage.WhetherGrids_ID, true,"是否栅格");
			CommonJQ.value(driver, AutoDTDataPage.GridsPre_ID, GridsPre,true,"栅格Pre");
			CommonJQ.value(driver, AutoDTDataPage.GridsLater_ID, GridsLater,true,"栅格Later");
		}
		SwitchDriver.switchDriverToSEQ(driver);
		ClusterOptimizationTask.setPara(driver, TextRSRP, TextSINR, TextPDCPUL,
				TextPDCPDL);
		ClusterOptimizationTask.chooseFile(driver, defaultWindowID, cfgfile, EParafile,
				DTfile);
		ClusterOptimizationTask.chooseFile2(driver, defaultWindowID, cfgfile2,
				EParafile2, DTfile2);
		String Name = AutoDTDataPage.setTaskName(driver, taskName);
		AutoDTDataPage.commitBtnClick(driver);
		return Name;
	...
	public static String creatCounterStatisticsTask(WebDriver driver,
		...
		SwitchDriver.switchDriverToFrame(driver, "//iframe");
		CommonJQ.click(driver, AutoDTDataPage.CounterStatistics_ID, true,"指标统计");
		CommonJQ.click(driver, AutoDTDataPage.ClusterOptimiz_ID, true,"簇优化统计");
